...
const Collection = ({category}) => {
  ...
  useEffect(() => {
    ...
    if (selectedSubCategory) {
      updated = updated.filter(
        (product) => product.subCategory?.toLowerCase() === selectedSubCategory
      );
    }
          ...
          >
            ...
            <div className="relative z-10 text-center text-white">
              ...
              <p className="mt-3 text-base md:text-lg tracking-wide">
                ...
                / <span className="text-white">{category ? `${category} Collection` : "All Collections"}</span>
              </p>
            </div>
          </div>
     <div className="flex flex-col sm:flex-row gap-6 md:pt-10 pt-4 border-t pb-8">
      {/* ✅ Left Sidebar Filters */}
      <div className="w-full sm:w-64 border-r pr-4 space-y-6 md:m-8 m-2">
        {/* Category */}
  ...
  {sizes.map((size) => (
    <div
      ...
      className={`border text-center  px-3 py-2 text-md cursor-pointer 
        ${selectedSize === size ? "bg-[#f8f7f4] text- border-black" : "bg-white hover:bg-gray-100"}`}
    >
      {size}
    </div>
  ))}
      ...
      <div className="flex-1">
        <div className="flex justify-between items-center text-base sm:text-md mb-4">
         <h1 className="text-[#5b4f47] font-bold text-lg">{category ? `${category} Collection` : "All Collections"}</h1>
        <select
  className="border border-gray-300 text-md px-2 py-1  mr-8  "
  value={sortOption}
...
<option value="ztoa">Sort by: Z - A</option>
  ...
  <option value="bestseller">Sort by: Bestseller</option>
        ...
        {/* Grid with 3 columns */}
       <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6 mr-4">
    ...
    filteredProducts.map((item) => (
      ...
      >
        ...
        <div className="bg-[#f8f7f4] p-2">
          ...
          <p className="text-md text-gray-500 text-center">${item.price}</p>
